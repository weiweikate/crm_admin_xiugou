        ...
        <div style="margin-top: 10px">
            ...
            <div class="check-area">
                ...
                <div v-if="second.length>0" class="data-area">
                    <div v-loading="loading" v-for="(item,index) in second" :key="index">
                        <el-checkbox @change="getThird(item,index)" v-model="secondChecked[index]"
                                     :value="item.id">
                        </el-checkbox>
            ...
            <div class="check-area">
                ...
                <div v-if="third.length>0" class="data-area">
                    <div v-loading="loading" v-for="(item,index) in third" :key="index">
                        <el-checkbox @change="getProductList(item,index)" v-model="thirdChecked[index]"
                                     :value="item.id">
                        </el-checkbox>
            ...
            <div style="margin-top: 10px">
                ...
                <div v-if="thirdClassifyTags.length">
                    三级类目：
                    <el-tag
                        v-for="(tag,k) in thirdClassifyTags"
                        :key="k"
    ...
    export default {
        ...
        methods: {
            ...
            isFirstSecondCheck(num) {
                const that = this;
                const temp = num == 1 ? this.secondChecked : this.productChecked;// 操作对象
                const preTemp = num == 1 ? this.firstChecked : this.secondChecked;// 操作对象的上一级
                const tempIndex = num == 1 ? this.firstIndex : this.secondIndex;// 操作对象上一级的索引值
                const preClassify = num == 1 ? this.first : this.second;// 操作对象上一级数据
                const classify = num == 1 ? this.second : this.productList;// 操作对象数据
                let count = 0;
                for (const i in temp) {
                    if (temp[i]) {
                        count++;
