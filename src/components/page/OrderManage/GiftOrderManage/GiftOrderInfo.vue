        ...
        <el-card :body-style="{ padding: '28px 50px' }">
            ...
            <div class="order-info">
                ...
                <p class="info-content">
                    ...
                    <span v-if='orderStatus != 1 && orderStatus != 2 ' class="content-con">支付交易号：{{ orderMsg.tradeNo }}</span>
                ...
                <el-table border :data="tableData" :span-method="spanMethod">
                    ...
                    <el-table-column prop="status" label="交易状态" align="center">
                        <template slot-scope="scope">
                            <template v-if='orderStatus == 1'>继续支付</template>
                            <template v-else-if='orderStatus == 2'>待发货</template>
                            <template v-else-if='orderStatus == 3'>待收货</template>
                            <template v-else-if='orderStatus == 4'>确认收货</template>
                            <template v-else-if='orderStatus == 5'>已完成</template>
                            <template v-else-if='orderStatus == 6'>已关闭(退货退款关闭)</template>
                            <template v-else-if='orderStatus == 7'>已关闭(用户关闭)</template>
                            <template v-else-if='orderStatus == 8'>已关闭(超时关闭)</template>
                        </template>
                    </el-table-column>
        ...
        methods: {
            ...
            getInfo() {
                request.orderDetail({ id: this.orderId }).then(res => {
                    ...
                    this.orderMsg.starIndex = res.data.adminStars || 1;
                    this.orderMsg.star =
                    ...
                    this.orderMsg.deliveryTime = res.data.finishTime; // 确认时间
                    this.orderMsg.sendTime = res.data.sendTime; // 发货时间
                    // this.order.cancleTime = res.data.sendTime; // 取消时间
                    this.orderMsg.tradeNo = res.data.outTradeNo;
                    ...
                    for (let i = 0; i < res.data.orderProductList.length; i++) {
                        res.data.orderProductList[i].orderProductPriceList.forEach((v, k) => {
                            ...
                            v.dealerPhone =
                                res.data.dealerPhone == null ? '/' : res.data.dealerPhone;
