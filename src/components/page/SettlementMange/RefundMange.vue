<template>
    <div class="refund-mange">
      <v-breadcrumb :nav="['结算管理','退款审核']"></v-breadcrumb>
      ...
      <el-card style='margin-top:20px' :body-style="{ padding: '30px' }">
        <el-table v-loading="tableLoading" :data="table" border>
          ...
          <el-table-column prop='freight_price' label="邮费" align="center"></el-table-column>
          ...
          <el-table-column prop='refund_time' label="退款时间" align="center">
            <template slot-scope="scope" v-if='scope.row.refund_time'>
              {{scope.row.refund_time | formatDate}}
            </template>
          </el-table-column>
          ...
          <el-table-column label="要求" align="center">
            <template slot-scope="scope">仅退款</template>
          </el-table-column>
          <el-table-column prop='adminUser' label="处理人" align="center">
          </el-table-column>
          <el-table-column label="处理情况" align="center">
            <template slot-scope="scope">同意退款</template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList(val){
      ...
      data.page = val;
      data.url = pApi.queryReturnMoneyList;
      this.tableLoading = true;
      this.$axios.post(api.queryReturnMoneyList,data).then(res=>{
