    ...
    <div class="profit-tpl">
        ...
        <el-card :body-style="{ padding: '30px' }" v-loading="pageLoading">
            ...
            <div class="option">
                ...
                <div class="my-inp">
                    设置：X= <el-input-number :controls="false" :min="0" @input="changePerMoney" v-model="shopMoney" placeholder="请输入数值"></el-input-number> %
                </div>
            </div>
            <div class="option">
            ...
            <div style="overflow:hidden">
                <div class="item">
                    <p>X值分配比例设置</p>
                    <el-checkbox style="margin:20px 0" v-model="checkX">拼店分配</el-checkbox>
                    <p v-for='(v,k) in formX' :key="k" style="overflow:hidden;line-height:30px">
                        <el-checkbox :label="k" class="smallItem" v-model="v.disable" @change="checked=>changeStatus(checked,'X',k)" >{{v.label}}</el-checkbox>
                        <el-input-number :controls='false' :min="0" :max="100" v-if='k!=0' :disabled="!v.disable" style="width:140px;float:right" size="mini" v-model="v.value"></el-input-number>
                        <el-input-number :controls='false' :min="0" :max="100" v-else v-for='(v1,k1) in v.value' :key="k1" :disabled="!v.disable" style="width:140px;float:right;margin-bottom:10px" v-model="v1.value"></el-input-number>
                        <!-- <el-input v-if='k!=0' :disabled="!v.disable" style="width:140px;float:right" v-model="v.value"></el-input> -->
                        <!-- <el-input v-else v-for='(v1,k1) in v.value' :key="k1" :disabled="!v.disable" style="width:140px;float:right;margin-bottom:10px" v-model="v1.value"></el-input> -->
                    </p>
                </div>
                <div class="item">
                    <p>Y值分配比例设置</p>
                    <el-checkbox style="margin:20px 0" v-model="checkY">拼店分配</el-checkbox>
                    <p v-for='(v,k) in formY' :key="k" style="overflow:hidden;line-height:30px">
                ...
                <div class="item">
                    <p>Z值分配比例设置</p>
                    <el-checkbox style="margin:20px 0" v-model="checkZ">拼店分配</el-checkbox>
                    <p v-for='(v,k) in formZ' :key="k" style="overflow:hidden;line-height:30px">
...
export default {
  ...
  data () {
    return {
        ...
        formX:[
            ...
            {disable:false,val:2,label:'店员',value:0}, // 店员
  ...
  activated(){
      ...
      if(this.tplId){
          ...
          this.url = api.addSettlementConfig;
      }
      this.cleanData();
  },
  deactivated(){
  ...
  methods: {
    ...
    beforeSubmit(){
        ...
        if(this.formX[0].disable){
            totalX += Number(this.formX[0].value[2].value);
        }
        this.formY.forEach((v,k)=>{
            if(k!=0){
                if(v.disable){
        ...
        if(this.formY[0].disable){
            totalY += Number(this.formY[0].value[2].value);
        }
        this.formZ.forEach((v,k)=>{
            if(k!=0){
                if(v.disable){
        ...
        if(this.formZ[0].disable){
            totalZ += Number(this.formZ[0].value[2].value);
        }
        if(totalX!=100 || totalY!=100 || totalZ!=100){
            console.log(totalX,totalY,totalZ);
    ...
    submitForm(){
        ...
        // 与位运算值
        data.hadJoin = this.lineVal;
        data.xCheck = this.valX;
        data.yCheck = this.valY;
        data.zCheck = this.valZ;
        data.id = this.tplId;
