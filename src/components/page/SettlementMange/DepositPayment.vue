<template>
    <div class="desposit-payment">
      ...
      <el-card style='margin-top:20px' :body-style="{ padding: '30px' }">
        <el-table :data="table" border v-loading="tabLoading">
          ...
          <el-table-column prop='amounts' label="缴纳金额" align="center">
              <template slot-scope="scope" v-if='scope.row.amounts'>
                  <span style="color:red">{{scope.row.amounts | handleMoney}}</span>
              </template>
          </el-table-column>
      ...
      <el-dialog title="保证金缴纳记录" :visible.sync="isShowdia" width="30%" >
          ...
          <p class="deposit-payment">缴纳类型：{{type}}</p>
          <p class="deposit-payment">支付余额：{{balance}}</p>
          <p class="deposit-payment">支付代币：{{tokenCoin}}</p>
          <p class="deposit-payment">三方支付：{{amounts}}</p>
...
export default {
  ...
  methods: {
    ...
    getList(val) {
      ...
      this.$axios
        .post(api.storePayRecordList, data)
        .then(res => {
