            ...
            <el-form :model="form" ref="form" label-width="100px">
                ...
                <el-form-item label="产品参数">
                    <div class="product-param">
                        <span v-if='productParm.length == 0'>暂无数据</span>
                        <span v-else v-for="(v,k) in productParm" :key="k">{{v.param}}：<el-input class="inp-param" v-model="v.paramValue"></el-input></span>
                    </div>
                </el-form-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            // 获取礼包信息
            getGiftInfo(){
                this.$axios.post(api.findGiftBagById,{id:this.giftId}).then(res=>{
                    this.form.name = res.data.data.name;
                    this.form.firstCategoryId = res.data.data.firstCategoryId;
                    this.form.secCategoryId = res.data.data.secCategoryId;
                    this.proItemArr.push(this.form.firstCategoryId);
                    this.proItemArr.push(this.form.secCategoryId);
                    this.getProItemId(this.proItemArr,true);
                    this.form.brandId = res.data.data.brandId;
                    this.getSupplyList();
                    this.form.supplier = res.data.data.supplierId;
                    this.form.originAddress = res.data.data.sendfrom.toString();
                    this.form.weight = res.data.data.weight;
                    this.form.freightTpl = res.data.data.freightTemplateId;
                    this.form.saleTime = res.data.data.aferServiceDays.toString();
                    this.form.editorContent = res.data.data.content;
                    this.form.pushWay = res.data.data.buyer.join(',');
                    this.productParm = res.data.data.paramList;
                    this.selectedTagArr = [];
                    res.data.data.tagList.forEach((v,k)=>{
                        let o = {};
                        o.label = '';
                        o.value = v;
                        this.selectedTagArr.push(o);
                    })
                    // 获取所有标签
                    this.getAllTags();
                    // 获取用户层级
                    this.checkedUsers = res.data.data.buyer;
                    ...
                    res.data.data.imgList.forEach((v,k)=>{
                        this.imgArr.push(v.original_img);
                        this.imgSmallArr.push(v.small_img);
                    })
                    //加载用户层级
                    this.getLevelList();
                })
            ...
            submitForm(){
                ...
                // 产品参数
                let paramTmp = [];
                this.productParm.forEach((v,k)=>{
                    paramTmp.push({paramId: v.paramId, value: v.paramValue})
                })
                data.specStr = JSON.stringify(paramTmp); 
                this.$axios.post(api.updateGiftBag,data).then(res=>{
