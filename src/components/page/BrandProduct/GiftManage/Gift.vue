        ...
        <el-card :body-style="{ padding: '20px 45px',color:'#666' }">
            ...
            <el-form :model="form" :rules="rules" ref="form" label-width="100px">
                ...
                <el-form-item label="礼包图片">
                    <el-upload class="img-uploader" :action="uploadImg" :show-file-list="false" :on-success="res=>successUpload(res,1)" v-if="videoUrl === ''">
                    ...
                    <el-upload class="img-uploader" :before-upload="beforeAvatarUpload" :action="uploadImg" :show-file-list="false" :on-success="successUpload" :disabled="isUseUpload" multiple>
                        <i class="el-icon-plus avatar-uploader-icon">添加图片</i>
                    </el-upload>
                    <div class="upload-tip">建议尺寸：800*800,拖拽图片可以改变顺序，第一张为默认头图</div>
                </el-form-item>
                <el-form-item label="产品分类">
                    <el-cascader @change='getProItemId' v-model="proItemArr" :options="itemList" :props="itemProps"></el-cascader>
                ...
                <el-form-item>
                    ...
                    <p v-if="isSetBuTime" style="margin: 10px 0 0 70px">可购买时间：
                        <el-date-picker v-model="setBuyTime" type="datetimerange" range-separator="至" start-placeholder="年 / 月 / 日" end-placeholder="年 / 月 / 日">
                        </el-date-picker>
                    </p>
                </el-form-item>
                ...
                <el-form-item label="商品详情">
                    ...
                    <el-upload class="fl" :action="uploadImg" multiple :before-upload="(file)=>beforeUploadImg(file, 'infoImg')" :on-success="uploadInfoImgSuccess" :show-file-list='false' list-type="picture-card">
                ...
                <div class="selected-tag">
                    ...
                    <el-tag class="tag" type="info" closable v-for="(v,k) in selectedTagArr" :key="k" @close="handleClose(k,v)">{{v.label}}
                    </el-tag>
                </div>
                <div class="add-tag">
                ...
                <div class="tag-list">
                    ...
                    <el-button style="margin-bottom:10px" v-for="(v,k) in tagArr" :key="k" @click="insertTag(v)" :disabled="v.selected" :class="{'selected-btn':v.selected}">{{v.label}}
                    </el-button>
                </div>
...
export default {
    ...
    data() {
        return {
            navList: {
                new: ['运营管理', '营销管理', '产品礼包管理', '新建礼包'],
                edit: ['运营管理', '营销管理', '产品礼包管理', '编辑礼包']
            },
            giftId: '',
            isUseUpload: false,
            ...
            aferServiceDays: [{ label: '无售后服务', value: '0' }, { label: '到货后7天', value: '7' }, { label: '到货后15天', value: '15' }, { label: '到货后30天', value: '30' }, { label: '到货后6个月', value: '180' }, { label: '到货后1年', value: '365' }, { label: '到货后2年', value: '730' }, { label: '到货后3年', value: '1095' }],
            itemProps: {
                value: 'value',
                children: 'children'
            },
            ...
            editorOption: {
                ...
                modules: {
                    // 配置富文本
                    toolbar: [['bold', 'italic', 'underline', 'strike'], ['blockquote', 'code-block'], [{ header: 1 }, { header: 2 }], [{ direction: 'rtl' }], [{ size: ['small', false, 'large', 'huge'] }], [{ header: [1, 2, 3, 4, 5, 6, false] }], [{ color: [] }, { background: [] }], [{ font: [] }], [{ align: [] }], ['clean'], ['link', 'image']]
                }
            },
            uploadData: {},
    ...
    computed: {
        ...
        nav() {
            return this.giftId ? this.navList['edit'] : this.navList['new'];
        }
    },
    ...
    methods: {
        ...
        getInfo() {
            request
                .findActivityPackageById({ packageId: this.giftId })
                .then(res => {
        ...
        beforeSubmit() {
            ...
            // const reg = /^(0|[1-9]\d*)([.]{1}[0-9]{1,2})?$/; // 判断是否是数字
            if (!regExpConfig.float.test(this.form.weight) || this.form.weight > 1000000000) {
        ...
        getAllTags(val, key, status) {
            ...
            request
                .querySysTagLibraryList({ typeId: val, secCategoryId: this.form.secCategoryId })
                .then(res => {
                    this.tagLoading = false;
                    this.tagArr = [];
                    this.tagTypeArr[key].selected = !this.tagTypeArr[key].selected;
                    res.data[0].sysTagLibraryVOList &&
        ...
        getAllTagType() {
            request
                .querySysTagTypePageList({ pageSize: 100000 })
                .then(res => {
                    this.tagTypeArr = [];
        ...
        handleClick(tab) {
            ...
            request
                .queryCouponByType({ type: tab.name })
                .then(res => {
