            ...
            <el-form>
                ...
                <el-form-item :label="`导航${k+1}`" label-width="130px">
                    <div class="del-area">
                        ...
                        <el-date-picker v-else v-model="v.navName" format="yyyy-MM-dd HH:mm" type="datetime" class="inp" @blur="changeTime" placeholder="选择日期时间">
                        </el-date-picker>
...
export default {
    ...
    data() {
        return {
            ...
            // 产品类型
            prodTypeList: [{ label: '降价拍', value: 2 }],
            totalNum: 0, // 字数统计
            topicNavbarList: [
                {
                    navName: '',
    ...
    methods: {
        ...
        //   添加产品
        addProduct(index) {
            this.topicNavbarList[index].topicBannerProducts ? this.topicNavbarList[index].topicBannerProducts.push({ prodCode: '', productType: 2 }) : (this.topicNavbarList[index].topicBannerProducts = [{ prodCode: '', productType: 2 }]);
            this.$set(this.topicNavbarList, index, this.topicNavbarList[index]);
        },
        //   获取输入字数
        getFontNum() {
