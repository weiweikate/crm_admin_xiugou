        ...
        <el-dialog :title="title" :visible.sync="mask" :before-close="closeDia">
            <el-form v-model="form" label-width="100px">
                ...
                <div class="inf-area" v-if="pageType!=8">
                    ...
                    <div class="tips">建议图片尺寸750px*460px</div>
                </div>
                <template v-if="pageType!=3&&form.linkType!=6&&form.linkType!=10">
                    <el-form-item label="请选择类型">
                        <el-select v-model="form.linkType" placeholder="请选择类型" @change="getName">
                            <el-option v-for="(v,k) in linkTypeList" :key="k" :label="v.type" :value="v.id">{{v.type}}</el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="请输入ID/地址">
                        <el-input v-model="form.linkTypeCode" placeholder="请输入ID/地址" @blur="getName" auto-complete="off"></el-input>
                    </el-form-item>
                </template>
                <template v-if="pageType!=3&&(form.linkType==6||form.linkType==10)">
                    <el-form-item label="请选择类型">
                ...
                <!-- <el-form-item v-if="pageType!=3&&pageType!=6&&pageType!=12" label="请输入ID">
                    <el-input v-model="form.linkTypeCode" placeholder="请输入ID" @blur="getName" auto-complete="off"></el-input>
                </el-form-item> -->
                ...
                <el-form-item v-if="pageType!=2&&pageType!=6&&pageType!=7&&pageType!=8&&pageType!=12" prop="date" label="投放时间">
                    <el-date-picker @blur="changeTime" class="input-style" v-model="form.date" type="datetimerange" range-separator="至" start-placeholder="开始时间" end-placeholder="结束时间"></el-date-picker>
                </el-form-item>
            </el-form>
        ...
        <div class="clear-mask" v-if="isShowClear">
            <div class="box">
                <div class="mask-title">
                    <icon class="ico" ico='icon-jinggao' /> 温馨提示</div>
...
export default {
    ...
    methods: {
        ...
        setConfig(status, pageType) {
            ...
            switch (pageType) {
                case 1:
                    navName = 'APP首页banner广告位管理';
                    break;
                case 2:
                    navName = 'APP首页推荐位管理';
                    break;
                case 3:
                    navName = 'APP首页明星店铺推荐位管理';
                    break;
                case 4:
                    navName = 'APP首页今日榜单广告位管理';
                    break;
                case 5:
                    navName = 'APP首页精品推荐广告位管理';
                    break;
                case 6:
                    navName = 'APP首页超值热卖专题广告位管理';
                    break;
                case 8:
                    navName = 'APP首页为你推荐广告位管理';
                    break;
                case 9:
                    navName = '拼店首页banner推荐位';
                    break;
                case 10:
                    navName = '类目搜索Banner广告位设置';
                    break;
                case 11:
                    navName = '秀场banner推荐图';
                    break;
        ...
        getList(val) {
            ...
            this.tableData = [];
            request
                .queryAdvertisementPageList(data)
                .then(res => {
