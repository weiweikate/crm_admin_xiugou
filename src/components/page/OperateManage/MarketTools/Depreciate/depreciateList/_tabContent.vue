<template>
    <div class="tab-content">
        <a href="#/addDepreciate" target="_blank" class="add-product el-button--small el-button el-button--primary" v-auth="'yunying.marketToolsManage.jjp.xjjjp'">新建降价拍</a>
        <div class="search-pane">
            <el-form :model="form" ref='form' inline label-width="100px" @submit.native.prevent>
                ...
                <el-form-item prop="startDate" label="起拍时间">
                    <el-date-picker v-model="form.startDate" format="yyyy-MM-dd HH:mm" type="datetimerange" start-placeholder="开始日期" end-placeholder="结束日期">
                    </el-date-picker>
                </el-form-item>
                <el-form-item prop="tstatus" label="结束方式">
                    <el-select v-model="form.dstatus" placeholder="全部">
                        <el-option v-for="item in endStyleArr" :key="item.value" :label="item.label" :value="item.value">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item prop="createUser" label="发布人">
                    <el-select v-model="form.createUser" placeholder="全部">
                        ...
                        <el-option v-for="item in createUserList" :key="item.id" :label="item.name" :value="item.id">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item prop="releaseDate" label="发布时间">
                    <el-date-picker v-model="form.releaseDate" type="datetimerange" format="yyyy-MM-dd" start-placeholder="开始日期" end-placeholder="结束日期">
                    </el-date-picker>
                </el-form-item>
        ...
        <el-table v-loading="tableLoading" :height="height" border :data="tableData" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="操作" align="center" min-width="100">
                <template slot-scope="scope">
                    <a :href="'#/depreciateInfo?id='+`${scope.row.id}`" target="_blank" class="el-button el-button--primary el-button--small" style="margin-bottom:10px" v-auth="'yunying.marketToolsManage.jjp.ck'">查看</a>
                    <el-button style="margin-bottom:10px" type="danger" @click="endOrDelete(1,scope.row)" v-if="scope.row.status === 1||scope.row.status === 2" v-auth="'yunying.marketToolsManage.jjp.js'">
                        结束
                    </el-button>
                    <el-button style="margin-bottom:10px" type="warning" @click="deInventory(scope.row)" v-if="scope.row.status === 2" v-auth="'yunying.marketToolsManage.jjp.jssl'">减少数量
                    </el-button>
                    <el-button style="margin-bottom:10px" type="danger" @click="endOrDelete(0,scope.row)" v-if="scope.row.status != 1&&scope.row.status != 2" v-auth="'yunying.marketToolsManage.jjp.sc'">
                        删除
                    </el-button>
...
export default {
    ...
    methods: {
        ...
        batchOperate(status) {
            ...
            request[url]({ list: this.multipleSelection.join(',') })
                .then(res => {
