<template>
    <div class="tab-content">
        <el-button @click="addPackage" v-if="p.addOperActivityPackage" style="margin-bottom: 15px" type="primary">新建套餐</el-button>
        <div class="search-pane">
            <el-form :model="form" ref="form" inline label-width="100px">
                ...
                <el-form-item prop="releaseDate" label="发布时间">
                    <el-date-picker v-model="form.releaseDate" type="datetimerange" format="yyyy-MM-dd" start-placeholder="开始日期" end-placeholder="结束日期"></el-date-picker>
                </el-form-item>
                <el-form-item prop="createAdmin" label="发布人">
                    <el-select v-model="form.createAdmin" placeholder="全部">
                        <el-option value>全部</el-option>
                        <el-option v-for="item in createUserList" :key="item.id" :label="item.name" :value="item.id"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label=" ">
        ...
        <el-table v-loading="tableLoading" border :data="tableData" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="套餐价格" align="center" min-width="50">
                <template slot-scope="scope">{{scope.row.originalPrice == undefined?'0':scope.row.originalPrice | formatPrice}}</template>
            </el-table-column>
            <el-table-column prop="productNumber" label="产品数量" align="center" min-width="50"></el-table-column>
            ...
            <el-table-column prop label="状态" align="center" min-width="50">
                <template slot-scope="scope">
                    ...
                    <template v-else-if="scope.row.status == 7">待审核
                        <br>(上架过)
                    </template>
                    <template v-else-if="scope.row.status == 8">已驳回
                        <br>(上架过)
                    </template>
                    <template v-else-if="scope.row.status == 9">待上架
                        <br>(上架过)
                    </template>
                </template>
            </el-table-column>
            <el-table-column label="操作" min-width="150" v-if="showOpr">
                <template slot-scope="scope">
                    <div style="float: left;margin-right: 5px;width: 80px">
                        <el-button type="danger" v-if="scope.row.status==1||scope.row.status==3||scope.row.status==6||scope.row.status==8&&p.updateOperActivityPackageStatusById_1" @click="packageStatus(scope.row,2)">提交审核</el-button>
                        <el-button type="danger" v-if="scope.row.status==2||scope.row.status==7&&p.updateOperActivityPackageStatusById_2" @click="packageStatus(scope.row,4)">通过审核</el-button>
                        <el-button type="warning" v-if="scope.row.status==2||scope.row.status==7&&p.updateOperActivityPackageStatusById_2" @click="packageStatus(scope.row,3)">驳回审核</el-button>
                        <el-button type="warning" v-if="scope.row.status==4||scope.row.status==9&&p.updateOperActivityPackageStatusById_3" @click="packageStatus(scope.row,5)">套餐上架</el-button>
                        <el-button type="warning" v-if="scope.row.status==5" @click="packageStatus(scope.row,6)&&p.updateOperActivityPackageStatusById_4">套餐下架</el-button>
                    </div>
                    <div style="float: left">
                        <div class="blue" v-if="scope.row.status!=2&&scope.row.status!=5&&scope.row.status!=7&&p.updateOperActivityPackage" @click="editPackage(scope.row)">套餐编辑</div>
                        <div class="blue" v-if="scope.row.status==1||scope.row.status==3||scope.row.status==4&&p.addPackageProduct" @click="productManage(scope.row)">产品管理</div>
                        <div class="blue" v-if="scope.row.status==5||scope.row.status==6||scope.row.status==8||scope.row.status==9&&p.updatePackageSpecValueStock" @click="addInventory(scope.row)">添加库存</div>
                        <div class="blue" v-if="scope.row.status==1||scope.row.status==3||scope.row.status==4&&p.updateOperActivityPackagePriceAndStock" @click="priceInventory(scope.row)">价格库存</div>
...
export default {
    ...
    methods: {
        ...
        priceInventory(row) {
            ...
            this.$axios
                .post(api.queryPackageProductList, { packageCode: row.packageCode })
                .then(res => {
