...
import { validateCouponName } from '@/utils/validate.js';
...
export default {
    ...
    data() {
        return {
            rules: {
                name: [{ trigger: 'blur', validator: validateCouponName }]
            },
                ...
                if (!valid) {
                    ...
                    if (!this.isDay) {
                        ...
                        if (!data.effectiveDays) {
                            return this.$message.warning('请输入可用周期数据!');
                        }
                        data.waitDays = 0;
                        data.cycleFlag = 0;
                        if (!reg.test(this.day)) {
                            return this.$message.warning('请输入1-4位数字的周期时间');
                        }
                    } else {
                    ...
                    if (that.remindFlag) {
                        ...
                        if (!data.remindDays) {
                            return this.$message.warning('请输入到期前提醒天数!');
                        } else {
                    ...
                    if (this.form.type === 4) {
                        if (!that.productList.products) {
                            return this.$message.warning('请选择可用产品!');
                        }
                        ...
                        if (firstCategoryIds.length === 0 && secondCategoryIds.length === 0 && thirdCategoryIds.length === 0 && productIds.length === 0) {
                            return this.$message.warning('请选择可用品类!');
                        }
                    ...
                    if (!this.checkedUsers) {
                        return this.$message.warning('请选择可使用用户层级!');
                    }
                    if (that.totalNumber) {
                        data.totalNumber = -1;
                    } else {
                        if (!data.totalNumber) {
                            return this.$message.warning('请输入发放数量!');
                        }
                    }
                    if (that.getLimit) {
                        data.getLimit = -1;
                        ...
                        if (!data.totalNumber) {
                            return this.$message.warning('请输入每人限额!');
                        }
                    }
                    if (this.useConditions) {
                        data.useConditions = this.useConditions;
