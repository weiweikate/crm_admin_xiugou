        ...
        <el-card :body-style="{ padding: '20px 40px' }" style='margin-top:20px'>
            ...
            <el-table :data="tableData" border>
                ...
                <el-table-column label="报损数" align="center">
                    <template slot-scope="scope">{{scope.row.lossCount||0}}件</template>
                </el-table-column>
...
export default {
    ...
    mounted() {
        this.warehouseCode = this.$route.query.repertotyId || sessionStorage.getItem('repertotyId');
        this.getDetail();
    ...
    methods: {
        ...
        showInfo(row) {
            this.$router.push({
                ...
                query: { repertoryInfoId: row.prodCode, warehouseCode: this.warehouseCode }
            });
        },
        // 获取仓库名 code
        getDetail() {
