        ...
        <el-dialog :title="action.title" :visible.sync="isShowModal">
            <el-form :model="modalData" label-width="100px" :rules="rules" ref="modalForm">
                ...
                <el-form-item label="备注说明" class="">
                    <el-input type="textarea" v-model="modalData.remark" auto-complete="off"
                ...
                <el-form-item label="授权" class="">
                    ...
                    <div class="s-list">
                        <div class="s-item" v-for="(item) in modalData.list" :key="item._id">
                            <el-input v-model="item.value" auto-complete="off" placeholder="请输入供应商编号"
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                rules: {
                    ...
                    status: [
                        { required: true, message: '请输入ERP状态', trigger: 'blur' }
                    ]
                },
                action: {
                    title: '添加ERP',
        ...
        methods: {
            ...
            disableErp(row) {
                ...
                let msg = status === 0 ? '启用成功' : '停用成功';
                this.loading = true;
                request.updateERPstatus({ id: row.id }).then(() => {
                    this.loading = false;
            ...
            transformSupplier(list) {
                let ts = +new Date(), count = 0, result = [];
                list.forEach(item => {
                    result.push({
                        _id: ts + '' + (++count),
                        value: item
            ...
            gotoEditModal(row) {
                this.loading = true;
                request.getERP({ id: row.id }).then(res => {
                    this.loading = false;
                    let data = res.data || {};
            ...
            transformModalData() {
                let tmp = [];
                let result = {};
                this.modalData.list && this.modalData.list.forEach(item => {
                    if (item.value) {
                        tmp.push(item.value);
                    }
                });
            ...
            // 获取数据
            getList(val) {
                let page = val || this.page.currentPage;
                const data = {
                    ...this.form,
                ...
                delete data.date;
                this.loading = true;
                request.getERPList(data).then(res => {
                    this.loading = false;
                    let data = res.data || {};
