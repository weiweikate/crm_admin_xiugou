<template>
    <div class="shareboun-account">
        ...
        <el-card :body-style="{ padding: '30px' }" style="margin-top:10px">
            ...
            <el-button @click="toggleStatus(2)" style="margin:10px 0" :type="status==2 || status==3?'primary':''">支出</el-button>
            <br>
            ...
            <el-table v-show="status == 1" :data="tableData" border v-loading="tableLoading">
                ...
                <el-table-column prop="balance" label="收入金额" align="center">
                    <template slot-scope="scope" v-if="scope.row.balance">{{`+${scope.row.balance}`}}</template>
                </el-table-column>
                <el-table-column prop="token_coin" label="收入代币（枚）" align="center">
                    <template slot-scope="scope" v-if="scope.row.token_coin">{{`+${scope.row.token_coin}`}}</template>
                </el-table-column>
                ...
                <el-table-column prop="create_time" label="时间" align="center">
                    <template slot-scope="scope" v-id="scope.row.create_time">{{scope.row.create_time | formatDate}}</template>
                </el-table-column>
            ...
            <el-table v-show="status == 3" :data="tableData" border v-loading="tableLoading">
                ...
                <el-table-column prop="token_coin" label="赠送代币（枚）" align="center">
                    <template slot-scope="scope">{{`-${scope.row.token_coin}`}}</template>
                </el-table-column>
                <el-table-column label="交易方式" align="center">
                    <template slot-scope="scope">待提现账户提现</template>
                </el-table-column>
                <el-table-column prop="create_time" label="时间" align="center">
                    <template slot-scope="scope" v-id="scope.row.create_time">{{scope.row.create_time | formatDate}}</template>
                </el-table-column>
